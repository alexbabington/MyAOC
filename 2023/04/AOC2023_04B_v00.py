...
###Initial Conditions
card_wins = []#Find no of wins on each card
...
for i_c,card in enumerate(cards):#Evaluates each card individually to determine score
    ...
    card_wins.append(no_of_wins)

#print(card_wins)

no_of_cards_arr = np.ones(len(card_wins))

for i_d,no_of_wins in enumerate(card_wins):#Iterates through and updates number of cards
    no_of_cards = no_of_cards_arr[i_d]
    
    i_e = 1
    while i_e <= no_of_wins:
        no_of_cards_arr[i_d+i_e] += no_of_cards
        i_e += 1

#print(no_of_cards_arr)

result = sum(no_of_cards_arr)
